...
function renderDetail(r) {
  ...
  detail.innerHTML = `
    ...
    <div class="section-title">${t('fieldIngredients')}</div>
  ...
  // Feld-Änderungen sammeln und beim Verlassen speichern
  const save = () => {
...
el('import-btn').addEventListener('click', async () => {
  ...
  btn.disabled = true;
  btn.textContent = t('importBtnLoading');
  state.job.recipes.forEach((r) => {
    if (r.selected) r.import_status = 'importing';
  });
  renderRecipeList();
  ...
  try {
    const res = await fetch(`/api/jobs/${state.jobId}/import`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ recipe_ids: selectedIds, cookbook_name: cookbookName }),
    });
    const data = await res.json();
