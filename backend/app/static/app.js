...
async function checkTandoor() {
  ...
  const checkDupBtn = el('check-duplicates-btn');
  try {
    ...
    if (data.connected) {
      ...
      checkDupBtn.classList.remove('hidden');
    } else {
      ...
      checkDupBtn.classList.add('hidden');
    }
  } catch {
...
el('check-duplicates-btn').addEventListener('click', async () => {
  const btn = el('check-duplicates-btn');
  const body = el('duplicates-modal-body');
  const originalLabel = btn.textContent;
  btn.disabled = true;
  btn.textContent = t('checkingDuplicates');
  body.innerHTML = `<p class="duplicates-empty">${t('checkingDuplicates')}</p>`;
  el('duplicates-modal').classList.remove('hidden');

  try {
    const res = await fetch('/api/tandoor/duplicates');
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || t('duplicatesCheckFailed'));
    }
    const data = await res.json();
    renderDuplicates(data);
  } catch (e) {
    body.innerHTML = `<p class="duplicates-empty">${escapeHtml(`${t('duplicatesCheckFailed')}: ${e.message}`)}</p>`;
  } finally {
    btn.disabled = false;
...
el('duplicates-modal-close-btn').addEventListener('click', () => {
...
function renderDuplicates(data) {
  const body = el('duplicates-modal-body');
  const categories = [
    { key: 'food', label: t('duplicatesCategoryFood') },
    { key: 'unit', label: t('duplicatesCategoryUnit') },
    { key: 'keyword', label: t('duplicatesCategoryKeyword') },
  ];

  const totalGroups = categories.reduce((sum, c) => sum + (data[c.key] || []).length, 0);
  if (totalGroups === 0) {
    body.innerHTML = `<p class="duplicates-empty">${t('duplicatesNoneFound')}</p>`;
    return;
  }
  ...
  body.innerHTML = categories
    ...
    .map((c) => {
      const rows = data[c.key].map((group) => {
        const names = group.items
          .map((item) => `<span class="dup-name">${escapeHtml(item.name)}</span><span class="dup-id">#${item.id}</span>`)
          .join(' ');
        return `<div class="duplicate-name-row">${names}</div>`;
      }).join('');
      return `<div class="duplicates-group"><div class="duplicates-category">${escapeHtml(c.label)} (${data[c.key].length})</div>${rows}</div>`;
    })
    .join('');
}
...
async function uploadFiles(files) {
  ...
  formData.append('force_ocr', el('force-ocr-checkbox').checked ? 'true' : 'false');
