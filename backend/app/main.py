...
from . import image_gen, jobs, tandoor_client
...
@app.post("/api/upload")
async def upload_files(files: list[UploadFile] = File(...), force_ocr: bool = Form(False)):
    ...
    jobs.save_job(job)
    threading.Thread(target=_run_extraction, args=(job.id, source_paths, doc_type, force_ocr), daemon=True).start()
...
@app.get("/api/tandoor/duplicates")
async def tandoor_duplicates():
    """Diagnostic: finds food/unit/keyword names that exist more than once in
    Tandoor under different casing/whitespace (e.g. "Schwarzer Pfeffer" and
    "schwarzer Pfeffer") - the exact situation that can otherwise crash an
    import (see tandoor_client._get_or_create)."""
    try:
        with tandoor_client.get_client() as client:
            return {
                "food": tandoor_client.find_case_insensitive_duplicates(client, "food"),
                "unit": tandoor_client.find_case_insensitive_duplicates(client, "unit"),
                "keyword": tandoor_client.find_case_insensitive_duplicates(client, "keyword"),
            }
    except tandoor_client.TandoorError as exc:
