...
def find_case_insensitive_duplicates(client: httpx.Client, endpoint: str) -> list[dict]:
    """Groups all items of a Tandoor list endpoint by their case/whitespace-
    normalized name and returns only the groups with more than one entry -
    i.e. likely-unintentional duplicates such as "Schwarzer Pfeffer" and
    "schwarzer Pfeffer" existing as two separate rows. This is exactly the
    situation that can make imports crash (see tandoor_client._get_or_create's
    docstring), so surfacing it lets the user clean it up in Tandoor directly."""
    items = fetch_all_items(client, endpoint)
    ...
    duplicates = [
        {"normalized": key, "items": sorted(group, key=lambda i: i["id"])}
        for key, group in groups.items()
        if len(group) > 1
    ]
    duplicates.sort(key=lambda d: d["normalized"])
    return duplicates
