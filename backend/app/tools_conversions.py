...
def run_scan(job_id: str) -> None:
    ...
    try:
        ...
        with tandoor_client.get_client() as client:
            job.progress_label = "Loading ingredients, units and existing conversions..."
            tool_jobs.save_tool_job(job)
            ...
            def target_of(food):
                ...
                return units.get(_ref_id(ref)) if ref else units_by_name.get("g")
            ...
            tool_jobs.save_tool_job(job)
            pairs = {}  # (food_id, unit_id) -> number of recipes using it
            for recipe in fetch_all_recipes_full(client):
            ...
            for (food_id, unit_id), count in sorted(pairs.items(), key=lambda kv: -kv[1]):
                ...
                if not food or not unit or not food.get("properties"):
                ...
                if metric and _key(metric[0]) == _key(target["name"]):
                    ...
                    continue
                to_estimate.append({"food": food, "unit": unit, "target": target, "count": count})

            job.progress_total = len(to_estimate)
            job.cost_estimate = (f"{len(to_estimate)} ingredient/unit pair(s) to estimate -> "
            ...
            for b, batch in enumerate(chunked(to_estimate, BATCH_SIZE), 1):
                ...
                for i, p in enumerate(batch):
                    ...
                    suggestions.append(ToolSuggestion(
                        ...
                        detail={"food": {"id": p["food"]["id"], "name": p["food"]["name"]},
